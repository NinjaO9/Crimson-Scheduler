...
class JsonStorageHandler:
    ...
    @classmethod
    def writeCourseApi(cls, collected_data: list[Campus], output_root: str | Path = "crimsonscheduler/public/api/v1/courses") -> list[Path]:
        ...
        for campus in collected_data:
            for semester in campus.semesters:
                ...
                output_path = (
                    output_root
                    / f"{cls.campusTermSlug(campus.name, semester.name)}.json"
                )
    ...
    def serializeSection(cls, section: Section, primary_course: Section) -> dict[str, Any]:
        ...
        return {
            ...
            "dates": {
                ...
                "lastAdd": cls.clean(metadata.get("dateLastStdAdd")),
                "lastDrop": cls.clean(metadata.get("dateLastStdDrop")),
                "lastWithdraw": cls.clean(metadata.get("dateLastWdrwl")),
            },
            "details": {
                "courseDescription": cls.clean(metadata.get("courseDescription")),
                "coursePrerequisite": cls.clean(metadata.get("coursePrerequisite")),
                "sectionComment": cls.clean(metadata.get("sectionComment")),
                "sectionTitle": cls.clean(metadata.get("sectionTitle")),
                "sectionUrl": cls.clean(metadata.get("sectionUrl")),
                "footnotes": cls.clean(metadata.get("footnotes")),
                "site": cls.clean(metadata.get("site")),
                "instructionMode": cls.clean(metadata.get("instructionMode")),
                "session": cls.clean(metadata.get("session")),
                "consent": cls.clean(metadata.get("consent")),
                "ucore": cls.clean(metadata.get("ucore")),
                "ger": cls.clean(metadata.get("ger")),
                "diversity": cls.clean(metadata.get("diversity")),
                "writing": cls.clean(metadata.get("writing")),
            },
        }
