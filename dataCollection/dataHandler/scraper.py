...
class Scraper:
    ...
    def fetch_data(cls, url) -> list[Campus]:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(
                ...
                )  # Wait for content to load (Content that contains the campuses, which inturn contains everything else)
            )

            html = driver.page_source
            soup = BeautifulSoup(html, 'html.parser').find(
                'div', class_='schedule-container'
            )  # Get the main container that holds all the data
                ...
                for semester in semesters:
                    ...
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (
                                By.CSS_SELECTOR,
                                f'a.nav-main[aria-label="{campus_name + semester.replace(" ", "")}"]',
                            )
                        )  # Wait for the table that contains the subjects to load
                    )

                    buttonToClick = driver.find_element(
                        By.CSS_SELECTOR,
                        f'a.nav-main[aria-label="{campus_name + semester.replace(" ", "")}"]',
                    )
                    try:
                        buttonToClick.click()
                    except Exception:  # Button not clickable
                        continue
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (
                                By.CSS_SELECTOR,
                                'table.wsu-c-table.wsu-c-table--striped.soc-table tbody tr',
                            )
                        )  # Wait for the table that contains the subjects to load
                    )
                    ...
                    html = driver.page_source
                    soup = BeautifulSoup(html, 'html.parser').find(
                        'tbody', class_=['wsu-c-table', 'wsu-c-table--striped']
                    )  # Get the table that contains the subjects
                    ...
                    for name, title in subjects:
                        ...
                        WebDriverWait(driver, 10).until(
                            EC.presence_of_element_located(
                                (By.CSS_SELECTOR, f'a[aria-label="{title}"]')
                            )
                        )
                        ...
                        WebDriverWait(driver, 10).until(EC.element_to_be_clickable(buttonToClick))
                        ...
                        WebDriverWait(driver, 10).until(
                            EC.presence_of_element_located(
                                (By.CLASS_NAME, 'tocsvbutton')
                            )  # Wait for the button that lets us access the csv to load
                        )

                        html = driver.page_source
                        soup = BeautifulSoup(html, 'html.parser').find('a', class_='tocsvbutton')
                        href = soup['href'].replace(
                            'tocsv', ''
                        )  # We do not want the csv, we want the raw html data
                        driver.get(href)

                        WebDriverWait(driver, 10).until(
                            EC.presence_of_element_located(
                                (By.CLASS_NAME, 'json-formatter-container')
                            )  # Wait for the table that contains the classes to load
                        )

                        html = driver.page_source
                        soup = BeautifulSoup(html, 'html.parser').find('pre')
                        classes = json.loads(soup.text)[
                            'sections'
                        ]  # Get the json data that contains the classes
                        classes_l = cls.formatClasses(
                            classes
                        )  # Format the classes into Section objects
    ...
    def getCampuses(cls, htmlData: BeautifulSoup) -> list:
        campus_info = htmlData.find_all(
            'div', class_='header_wrapper'
        )  # Get all wrappers that contain campus names and semesters
        campuses = []
    ...
    @classmethod
    def getSemesters(
        cls, campusData: BeautifulSoup
    ) -> list:  # Return string because there is no information nested under semesters - it is a link that changes the page
        semestersList = campusData.find('ul', class_='Semesters').find_all(
            'li', class_='nav-item semester'
        )
        semesters = [
            semester.find('a', class_='nav-main').text.strip() for semester in semestersList
        ]
