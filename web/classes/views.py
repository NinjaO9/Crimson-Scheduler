...
from classes.models import Campus, ScheduleSection, Section, UserSchedule
...
def add_to_schedule(request, section_id):
    ...
    try:
        section = Section.objects.select_related(
            'course', 'course__topic', 'course__topic__semester', 'course__topic__semester__campus'
        ).get(id=section_id)
        ...
        user_schedule, _ = UserSchedule.objects.get_or_create(
            session_id=session_id, semester=semester
        )
        ...
        ScheduleSection.objects.create(
            schedule=user_schedule, section=section, has_conflict=has_conflict
        )
        ...
        if has_conflict:
            for conflict_section in conflicts:
                conflict_schedule_section = ScheduleSection.objects.get(
                    schedule=user_schedule, section=conflict_section
                )
...
def remove_from_schedule(request, section_id):
    ...
    try:
        ...
        semester = section.course.topic.semester
        user_schedule = UserSchedule.objects.get(session_id=session_id, semester=semester)
...
def get_schedule_calendar(request, user_schedule):
    ...
    schedule_sections = user_schedule.schedule_sections.select_related(
        'section', 'section__course', 'section__course__topic'
    ).all()
    ...
    return JsonResponse({'success': True, 'schedule': calendar_data})
    ...
    try:
        ...
        for section_id in section_ids:
            ...
            grouped_section_ids = [
                ...
                and sections_by_id[candidate_id].course_id == section.course_id
            ]
            schedule_group_id = (
                '-'.join(grouped_section_ids)
                if course_counts.get(section.course_id, 0) > 1
                else None
            )
